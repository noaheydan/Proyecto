<template>
  <v-container>
    <v-app-bar app color="primary" dark>
      <v-btn text to="/profile">Inicio</v-btn>
    </v-app-bar>
    ...
    <v-row justify="center">
      <v-col md="6" sm="6">
       <v-card v-if="suForm">
          <v-card-title primary-title class="justify-center">Completar los campos</v-card-title>
          <v-card-text>
            <v-form class="ma-3" @submit.prevent="signup()" ref="signupForm">
              ...
              <v-btn block class="primary mt-3" type="submit">Guardar</v-btn>
            </v-form>
          </v-card-text>
        </v-card>
        <v-card v-else>
